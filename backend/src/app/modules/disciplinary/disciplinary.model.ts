...
disciplinaryActionSchema.methods.escalate =
  async function (): Promise<IDisciplinaryActionDocument> {
    ...
    if (escalationMap[currentSeverity]) {
      this.severity = escalationMap[currentSeverity] as
        | "low"
        | "medium"
        | "high"
        | "critical";
...
disciplinaryActionSchema.methods.getEscalationLevel = function (): string {
  ...
  return (
    escalationLevels[this.severity as keyof typeof escalationLevels] ||
    "Unknown"
  );
};
...
disciplinaryActionSchema.methods.addFollowUp = async function (
...
// Static methods
disciplinaryActionSchema.statics.findByStudent = function (
  studentId: string,
  filters: any = {}
) {
...
disciplinaryActionSchema.statics.findByTeacher = function (
  teacherId: string,
  filters: any = {}
) {
...
disciplinaryActionSchema.statics.findBySchool = function (
  schoolId: string,
  filters: any = {}
) {
...
disciplinaryActionSchema.statics.getStudentDisciplinaryHistory =
  async function (studentId: string) {
    ...
    const totalPoints = actions.reduce(
      (sum: number, action: any) => sum + (action.points || 0),
      0
    );
    const activeActions = actions.filter(
      (action: any) => action.status === "active"
    ).length;
    const redWarrants = actions.filter(
      (action: any) => action.isRedWarrant
    ).length;

    return {
      totalActions: actions.length,
      activeActions,
      resolvedActions: actions.filter(
        (action: any) => action.status === "resolved"
      ).length,
      totalPoints,
      redWarrants,
      ...
      categoryBreakdown: {
        ...
        attendance: actions.filter((a: any) => a.category === "attendance")
          .length,
        academic: actions.filter((a: any) => a.category === "academic").length,
        discipline: actions.filter((a: any) => a.category === "discipline")
          .length,
...
disciplinaryActionSchema.statics.escalateWarning = async function (
  actionId: string,
...
disciplinaryActionSchema.statics.getDisciplinaryStats = async function (
  schoolId: string,
  filters: any = {}
): Promise<IDisciplinaryStats> {
  ...
  return {
    ...
    bySeverity: {
      low:
        stats?.severityBreakdown.filter((s: string) => s === "low").length || 0,
      medium:
        stats?.severityBreakdown.filter((s: string) => s === "medium").length ||
        0,
      high:
        stats?.severityBreakdown.filter((s: string) => s === "high").length ||
        0,
      critical:
        stats?.severityBreakdown.filter((s: string) => s === "critical")
          .length || 0,
    },
    byCategory: {
      behavior:
        stats?.categoryBreakdown.filter((c: string) => c === "behavior")
          .length || 0,
      attendance:
        stats?.categoryBreakdown.filter((c: string) => c === "attendance")
          .length || 0,
      academic:
        stats?.categoryBreakdown.filter((c: string) => c === "academic")
          .length || 0,
      discipline:
        stats?.categoryBreakdown.filter((c: string) => c === "discipline")
          .length || 0,
      uniform:
        stats?.categoryBreakdown.filter((c: string) => c === "uniform")
          .length || 0,
      other:
        stats?.categoryBreakdown.filter((c: string) => c === "other").length ||
        0,
    },
    byGrade: [],
    recentTrends: [],
...
disciplinaryActionSchema.statics.getOverdueActions = function (
  schoolId: string
) {
  return this.find({
    schoolId,
...
disciplinaryActionSchema.statics.generateDisciplinaryReport = async function (
  schoolId: string,
  filters: any
) {
...
export const DisciplinaryAction = model<
  IDisciplinaryActionDocument,
  IDisciplinaryActionModel
>("DisciplinaryAction", disciplinaryActionSchema);

export const Punishment = model<IPunishmentDocument>(
  "Punishment",
  punishmentSchema
);
export const RedWarrant = model<IRedWarrantDocument>(
  "RedWarrant",
  redWarrantSchema
);
